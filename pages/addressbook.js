...
const AddressBook = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`${baseUrl}/api/deleteaddress`, {
        data: id,
      });
      if (response.data.success === true) {
        setUserAddress(
          userAddress.filter((item, index) => {
            return item._id !== id;
          })
        );
        ...
        <div className="innerDiv flex flex-row py-6">
          ...
          <div className="main w-[75%]">
            <div className="header flex justify-between">
              ...
              <h1
                className="font-firasans text-base lg:text-lg text-orange-900 flex space-x-2 justify-center items-center cursor-pointer hover:text-blue-500"
