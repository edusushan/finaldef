...
const Checkout = ({
  ...
  const fetchAddress = async () => {
    const response = await axios.get(`${baseUrl}/api/fetchaddress`, {
      headers: {
  ...
  return (
    <div className="container mx-auto max-w-[1200px] px-3 mb-20">
      <h1 className="font-bold text-3xl my-8 text-orange-800 text-center font-roboto">
        Checkout
      </h1>
      ...
      <div className="sidebar bg-orange-100 px-10 py-4 rounded-md font-medium">
      ...
      <div className="ml-3 flex space-x-4">
        <button
          disabled={disabled}
          onClick={async () => {
            const response = await axios.post(`${baseUrl}/api/pretransaction`, {
              data: orderDetails,
            });
            // console.log(response.data);
          ...
          className="transition-all duration-300 disabled:bg-orange-200 disabled:shadow-none disabled:text-black font-firasans bg-orange-400 py-1 my-2 text-lg px-10 md:px-5 font-medium text-center rounded-md hover:bg-orange-500 flex items-center justify-center space-x-2 shadow-lg shadow-gray-700/50 text-purple-700"
        >
